...
const storage = new CloudinaryStorage({
    ...
    params: async (req, file) => {
        // Tách tên file gốc và phần đuôi (extension như rar, zip, pdf...)
        ...
        // Dọn dẹp tên (Bỏ tiếng việt có dấu, dấu cách) để làm đường dẫn URL chuẩn hơn
        const cleanName = baseName.replace(/[^a-zA-Z0-9]/g, "_").substring(0, 50);

        // Phân biệt file đa phương tiện và file tài liệu
        const isMedia = file.mimetype.startsWith("image/") || file.mimetype.startsWith("video/");

        return {
            folder: "taska_attachments",
            resource_type: "auto",
            public_id: isMedia ? `${cleanName}_${Date.now()}` : `${cleanName}_${Date.now()}.${extension}`
        };
    }
});
