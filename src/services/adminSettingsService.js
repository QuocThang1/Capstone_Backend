...
const editableFields = Object.keys(defaultSettings).filter((field) => field !== "key");
...
const normalizeSettings = (settings) => {
    ...
    editableFields.forEach((field) => {
        ...
        result[field] = settings[field] === undefined ? defaultSettings[field] : settings[field];
    });
    return result;
};
...
const validateSettings = (settings) => {
    if (!settings.platformName?.trim()) throw new ApiError(StatusCodes.BAD_REQUEST, "Platform name is required");
    if (!emailPattern.test(settings.supportEmail || "")) throw new ApiError(StatusCodes.BAD_REQUEST, "Support email is invalid");
    if (!emailPattern.test(settings.senderEmail || "")) throw new ApiError(StatusCodes.BAD_REQUEST, "Sender email is invalid");
    if (!settings.allowPasswordLogin && !settings.allowThirdPartyLogin) {
        throw new ApiError(StatusCodes.BAD_REQUEST, "At least one login method must remain enabled");
    }
    if (settings.maxLoginAttempts < 1 || settings.maxLoginAttempts > 20) {
        throw new ApiError(StatusCodes.BAD_REQUEST, "Max login attempts must be between 1 and 20");
    }
    if (settings.lockAccountDurationMinutes < 1 || settings.lockAccountDurationMinutes > 1440) {
        throw new ApiError(StatusCodes.BAD_REQUEST, "Lock duration must be between 1 and 1440 minutes");
    }
    if (settings.sessionTimeoutMinutes < 5 || settings.sessionTimeoutMinutes > 1440) {
        throw new ApiError(StatusCodes.BAD_REQUEST, "Session timeout must be between 5 and 1440 minutes");
    }
    if (settings.warningThresholdHours < 1) {
        throw new ApiError(StatusCodes.BAD_REQUEST, "Warning threshold must be at least 1 hour");
    }
    if (settings.criticalThresholdHours < settings.warningThresholdHours) {
        throw new ApiError(StatusCodes.BAD_REQUEST, "Critical threshold must be greater than or equal to warning threshold");
    }
};

const getSystemSettingsService = async () => normalizeSettings(await getOrCreateSystemSettings());
...
const getPublicAuthSettingsService = async () => {
    ...
    return {
        ...
        allowThirdPartyLogin: settings.allowThirdPartyLogin ?? settings.allowGoogleLogin ?? false,
    };
};
...
const updateSystemSettingsService = async (data, updatedBy) => {
    ...
    const updateData = {};
    editableFields.forEach((field) => {
        updateData[field] = data[field] === undefined
            ? (field === "allowThirdPartyLogin" && existing[field] === undefined
                ? (existing.allowGoogleLogin ?? defaultSettings[field])
                : (existing[field] === undefined ? defaultSettings[field] : existing[field]))
            : data[field];
    });
...
const sendSystemTestEmail = async ({ to, subject, message }) => {
    ...
    if (!emailPattern.test(recipient || "")) throw new ApiError(StatusCodes.BAD_REQUEST, "Recipient email is invalid");

    try {
        const testEmail = await renderEmailTemplate(EMAIL_TEMPLATE_KEYS.SYSTEM_TEST, {
